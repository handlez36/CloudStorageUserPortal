...
// import { data2 } from './FakeGraphData';
import { smallDataInGB as data2 } from './FakeGraphData2';
import { StorageApi } from '../../../services/storage';
import { LOCATION_THEME } from '../StorageConstants';
...
class StorageUsageGraph extends Component {
	...
	render() {
		...
		return (
			<div className={`storage-usage-graph ${graphType}`}>
				{data && (
					<Fragment>
						<div className={'graph-key'}>
							<div className='graph-type-section'>
								{commitmentAmount !== 'undefined' && (
									<div className='title'>{commitmentAmount}</div>
								)}

								<img src={SHARE_TYPE_ICONS[shareType] || ObjectStorageIcon} className='type-icon' />
							</div>
							<div className='location-icon-row'>
								{graphType === 'singleShare'
									? StorageUtils.getLocationIconsOneShare(share)
									: StorageUtils.getLocationIcons(data)}
							</div>
						</div>
						<div className={'chart-wrapper'}>
							<Victory.VictoryChart
								// containerComponent={<Victory.VictoryZoomContainer />}
								theme={{ ...Victory.VictoryTheme.grayscale, ...theme }}
								minDomain={minDomain}
								maxDomain={maxDomain}
